  ...
  try {
    ...
    if (!requestType || !data) {
      return res.status(400).json({
        success: false,
        message: 'Please provide requestType and data'
      });
    }

    // Validate request type
    if (!['create', 'edit', 'delete'].includes(requestType)) {
      return res.status(400).json({
        success: false,
        message: 'Invalid request type. Must be create, edit, or delete'
      });
    }
...
const handleEditApproval = async (itemId, data) => {
...
const handleDeleteApproval = async (itemId) => {
...
exports.approveRequest = async (req, res) => {
  try {
    ...
    // Populate the approvedBy field
    await approval.populate('approvedBy', 'name email role');
...
exports.rejectRequest = async (req, res) => {
  try {
    ...
    // Populate the rejectedBy field
    await approval.populate('rejectedBy', 'name email role');
